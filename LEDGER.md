...
**Last amended:** 2026-09-22 UTC (round-two review disposition)
...
- **evidence** — `reproduced` (executed against `a269d8d`), `source` (reachable code path), `chrome-docs`, `chromium-source`, `browser-needed`, `pending` (claimed but not yet independently confirmed), `design`.
...
AR-01  | configRevision serialization is ambiguous; delimiters legal inside field values are unescaped | open | high | v0.2.1/s2 | — | reproduced | —
AR-01b | Stale-edit and draft binding need a per-profile digest, not the configuration revision | open | high | v0.2.1/s2 | — | design | —
...
AR-04  | permissions.request() runs after unbounded storage, render, collision and permission work inside one user gesture | open | high | v0.2.1/s4 | HW-V6-10 | source, chrome-docs | DECIDE: explicit grant vs auto-prompt
...
AR-11  | Manifest floor 101 but storage.session requires 102; the test pins the obsolete number instead of deriving it | open | medium | v0.2.1/s5 | — | reproduced | —
...
DR-01  | Store listing reportedly says "what you see is what's applied", collapsing registered rule into wire effect | open | high | v0.2.1/s5 | — | pending | confirm listing text
...
AR-15  | 5,000-profile cap is the only input budget; file, field, domain, header, value and render budgets are absent | open | medium | slice:scale | HW-V6-10 | source | —
...
AR-16  | Export writes colliding configurations that the same build refuses to import | open | medium | slice:scope | FINDING-027, HW-V6-13 | reproduced | DECIDE: rescue vs portable
...
Until that coordinator exists, this is the supported contract, and slice 1 and slice 2 exist to make it true and tested:
...
- **AR-18 reframed.** "No behaviour tests" was false — `test/selftest.mjs` has executable queue and `configRevision` suites. The real defect is oracle completeness: `await q2(true)` in the queue suite is only safe because the queue is broken, and `configRevision` is tested for stability and change but never for injectivity. AR-18 is therefore a method row, not a phase.
- **AR-07 re-rated.** The round-one downgrade rested on two popups being impossible. That is true and irrelevant: one popup re-enters its own mutations because no action control is ever disabled. AR-07a is now v0.2.1/s1, high.
- **AR-01 split by role.** A value that decides whether a write is accepted has a different contract from one that decides whether a status line says "checking". Status freshness keeps canonical-encoded FNV, synchronous. Draft and stale-edit binding gets a per-profile SHA-256 — not because a crafted collision is a security threat in a local single-user tool, but because exact canonical text is the simplest correct binding and DR-02's 1 MB session quota at the Chrome 102 floor rules it out.
- **AR-04 is decision-gated.** The explicit-grant design removes the dependency on AR-09 that justified deferring it, so the decision belongs in v0.2.1. If auto-prompt is retained instead, its acceptance gate is a real-Chrome activation run at the maximum supported input, not a reordering.
...
- **AR-12 split into three.** WebTransport is HTTPS-only by constructor and so *is* expressible by the existing patterns; WebBundle's outer fetch is an ordinary URL fetch. Only WebSocket has the scheme-expressibility problem. One ruling for all three would have been wrong.
...
None needs code. Each one, unresolved, stalls a row.

1. **Grant flow** (AR-04) — explicit Grant control, or auto-prompt with a browser activation gate.
2. **PSL policy** (AR-06) — full dated snapshot used for warnings is the recommendation; confirm.
3. **WebSocket / WebTransport / WebBundle** (AR-12a/b/c) — pin an observed mapping per protocol, or remove that resource type.
4. **Export contract** (AR-16) — faithful rescue backup, guaranteed-importable portable config, or two named artifacts.
5. **Header-name casing** (F-029) — echo as typed, or canonical with the behaviour documented.
6. **R15 popup surface** — rule on it, or park it with a trigger condition.
7. **Version policy** (FEAT-2) — keep "minor adds one feature, patch is fixes only", or change it explicitly in `README.md` before response append lands.
8. **Storage budgets** (DR-02) — fixed product limits set independently of browser quota.
...
Checks 6 and 8 are the ones that matter. Six guards against history and current state drifting apart, which is AR-20's recurrence. Eight guards against a row being marked proven on the strength of a source scan, which is AR-18's.
